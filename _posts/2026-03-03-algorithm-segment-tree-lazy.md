...
普通线段树可以在 `O(log n)` 完成单点更新，但若区间 `[l,r)` 的每个元素都加 d，逐点更新仍要 `O((r-l)log n)`。真正需要延迟的是对子节点的重复工作：当更新完整覆盖一个节点区间时，当前查询只关心该区间总和，没有理由立刻访问每个叶子。

懒标记把“这个区间所有元素都应再加 d”作为节点状态保存。节点摘要立即更新，子节点细节等到确实需要下钻时再恢复。一次区间操作因此只沿两条边界路径和中间的完整覆盖节点工作。
...
```text
tree[p] = 节点区间在全部已接受更新后的真实总和
lazy[p] = 已计入 tree[p]、但尚未计入两个孩子的逐元素增量
```

所以有 lazy 不表示父节点的 sum 过期；过期的是孩子对这部分更新的认识。
...
若节点 `[0,2)` 保存 `[1,2]`，其总和为 3。完整执行 `range_add(0,2,10)` 时，两个元素各增加 10，所以父节点应直接从 3 变为 23：
...
此时两个叶子仍保存 1 和 2。后来若查询 `[1,2)`，`push` 才把 10 分别应用到左右叶子，清空父节点 lazy，再向右查询得到 12。延迟改变了执行时机，没有改变逻辑上的数组。

固定实验从 `[1,2,3,4,5]` 执行 `[1,4)+=10`，逻辑数组变为：
...
随机测试使用固定 seed `20260625`，执行 300 轮；每轮长度在 1 到 100 之间，并执行 500 次区间加法或区间求和，共 150000 次操作。线段树结果必须与朴素数组逐元素更新和求和一致。测试还覆盖空树的空区间操作和非法反向区间，能够检查局部覆盖、完全覆盖、无交集、空区间和多次叠加。
...
一个连续查询区间在树的每一层最多留下常数个边界节点，因此访问节点数为 `O(log n)`。若更新规则不能用常数大小标记组合，例如每个元素执行依赖其旧值的任意函数，这个复杂度保证不再自动成立。

## 输入和实现边界

- 实验接口要求 `0<=l<=r<=n`，非法范围会抛出异常；空区间的和为 0，空更新无效果。
- 空数组不构建根节点，只允许 `[0,0)`。
- `tree` 和 `lazy` 使用 `long long`；`delta*(r-l)` 仍可能在极端输入下溢出或溢出。
- 当前标记只有“加法”。若同时支持赋值与加法，必须定义标记组合顺序，不能把两个数字无条件相加。
...
**误以为 lazy 表示父节点过期。** `apply` 后父节点 sum 已正确；进入孩子前才需要 push。
