...
全栈项目必须回答一个基本问题：刷新页面后数据还在不在。把数据只放在浏览器变量或服务器内存里，重启后就会丢。JSON 文件持久化让这个问题变得可见：你可以打开 `tasks.json`，看到任务数组如何随 API 请求变化。
...
这条链路跑通后，再引入 SQLite 或数据库事务会更容易理解，因为数据库本质上也在管理持久状态，只是提供更强的查询、约束和并发能力。
...
`initial_count=2` 来自第一次 `GET /api/tasks`，说明服务器从 seed 文件中读到了两条任务。`created_id=3` 说明新增任务时，`nextId()` 扫描已有 id 后生成下一个 id。`final_total=2` 说明删除刚创建的任务后，文件中的任务数量回到两条。
...
smoke 结束后，文件中应保留两条 seed 任务。新增 id 为 3 的任务已经被删除，所以它只出现在 API transcript 中，不应出现在最终文件里。
...
```text
read:   tasks -> 原样返回
create: tasks -> push(newTask)
update: tasks -> 找到 id 后修改 done
delete: tasks -> filter 掉目标 id
```

每次变化后都调用 `saveTasks()` 写回完整数组。这种写法适合单用户本地 demo；并发写入和事务一致性留给后续数据库主题处理。
