...
迹和范数把扩域元素压回底域。这个压回操作让坐标提取、对偶基、序列构造和编码校验都能用底域线性代数表达。
...
```python
def trace16(a):
    return gf_add(a, gf_pow(a, 2, mod16_np), gf_pow(a, 4, mod16_np), gf_pow(a, 8, mod16_np))
trace_basis = [trace16(1), trace16(x), trace16(gf_pow(x, 2, mod16_np)), trace16(gf_pow(x, 3, mod16_np))]
assert trace_basis == [0, 1, 1, 1]
```

本次实验输出摘录：

```text
trace16_basis_values=[0, 1, 1, 1]
```

## 输出怎么读

输出说明基 `1, alpha, alpha^2, alpha^3` 的迹分别是 `0,1,1,1`。因此任意元素 `beta0+beta1 alpha+beta2 alpha^2+beta3 alpha^3` 的迹就是 `beta1+beta2+beta3`。

## 状态变化

状态变化是：扩域元素先被 Frobenius 反复作用；四个共轭项相加；结果被 Frobenius 固定，落回 `F_2`；线性性再把任意元素的迹化成坐标公式。

## 练习或延伸

选 `beta=alpha^3+alpha+1`，先用坐标公式算迹，再用 `a+a^2+a^4+a^8` 直接算一遍。
