...
在私有开发树的合成样例上重跑 suite 和测试，得到：

```text
RELEASE_INTEGRITY_SUITE decision=ready-for-human-review components=5
Ran 2 tests in 0.308s
OK
```

报告进一步记录 `blocking components=0`、ReproBadge `100/100`、claim ledger `ready 2 / blocked 0`、EvidencePack findings `0` 和 release gate blocking findings `0`。这些字段可以追回各组件报告，总览页没有自行重新判分。

### 阻断路径是否真的生效

只跑成功样例不足以验收发布门禁。另一轮隔离实验复制了合成样例，删除其 `LICENSE` 后重跑相同编排器：

```text
RELEASE_INTEGRITY_SUITE decision=block components=5
exit status: 1
Blocking components: 1
Public Repo Release Gate: missing_required_file LICENSE
```

这个结果展示了分层信号的作用：ReproBadge 仍然给出 `92/100, grade ready`，但 Public Repo Release Gate 对必需文件使用硬阻断，因此 suite 最终返回 `block`。综合分数不能覆盖硬约束，这也是保留 component report 的原因。
...
第一个误判是把 suite 总览页当成唯一事实来源。总览页只聚合组件状态；真正的阻断原因仍要回到 Agent Leak Sentinel、ReproBadge、Claim Ledger、EvidencePack 和 Release Gate 的组件报告。

第二个误判是用综合分数覆盖硬阻断。样例里删除 `LICENSE` 后，ReproBadge 仍可能给出较高材料分，但 release gate 必须阻断；硬约束不能被平均分稀释。

第三个误判是让 orchestrator 复制组件逻辑。suite 应该编排和汇总，组件才是判断来源。否则规则改动会在两个地方分叉，最终报告会失去可追溯性。

## 可以怎样练习

复制一份合成发布仓库，分别删掉 `LICENSE`、删掉 evidence 文件、加入一个疑似 token 形状字符串。每次只改一个条件，观察最终 decision 和组件报告怎样变化。练习目标是确认 suite 能解释“为什么阻断”，而不是只给一个失败状态。
