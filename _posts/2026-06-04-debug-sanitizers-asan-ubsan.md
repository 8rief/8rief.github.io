...
编译通过只说明语法和一部分静态规则成立。C++ 的很多错误发生在运行时：访问 vector 末尾之后的内存、释放后继续使用、整数溢出、违反对齐要求。普通运行可能没有立刻崩溃，结果却已经不可信。

Sanitizer 构建在程序里插入检查点，让这些危险行为在发生时停止并输出证据。它适合调试和 CI 中的专门检查，不等同于发布构建。
...
```text
unsafe_index_demo      触发 heap-buffer-overflow
signed_overflow_demo   触发 signed integer overflow
```

它们是教学用失败样例，真实项目中应修复后再发布。
...
非零退出说明 sanitizer 已经把危险行为转成可被脚本识别的失败。
...
2. 触发函数，例如 `unsafe_index_demo()`。
3. 源码位置，例如 `debug_lab.cpp` 中访问越界的行。

原始报告可能包含本机路径和内存地址，公开文档里只需要保留类别、函数和相对源码位置，避免泄露本地路径。
...
5. **看到 sanitizer 报错只改到不报错。** 还要补回归测试，证明同类输入纳入了测试覆盖。
