...
第一个误判是把黑色矩形当成删除。PDF 里可见对象、文本对象、批注、表单字段和 metadata 可以分层存在；屏幕上看不到，不代表文件结构里已经不存在。

第二个误判是把 `no-obvious-risk` 当成安全证明。这个状态只说明当前启发式规则没有命中；它不覆盖 OCR、复杂嵌入对象、加密容器、扫描质量或人工误标。

第三个误判是把报告直接转发给别人。报告应该只含规则名和位置级别的说明；如果报告复制了疑似敏感原文，预检工具本身就制造了新的泄漏面。

## 可以怎样练习

准备两个合成文件：一个 PDF 里保留文字但用黑框覆盖，另一个导出成扁平化 PNG。练习时先预测哪个会命中 `risk-found`，再看报告里有没有复制敏感内容。这个练习能帮助读者区分视觉层和文件结构层。
