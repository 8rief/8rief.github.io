...
很多数组题都在问“离当前位置最近的某个边界在哪里”。暴力扫描的慢点不在比较本身，而在同一个元素会被许多位置反复检查。例如找右侧第一个更大元素时，一个较小值在遇到真正能击败它的元素之前，会被后面的多个位置重复考虑。

单调栈引入了一个等待队列：还没找到边界的元素先入栈，新元素到来时统一结算被它击败的候选。每个元素只进入一次、离开一次，重复扫描被改写成一次弹栈事件。

这就是单调栈比普通栈多出的语义：栈内顺序代表一组尚未解决且仍有资格成为答案的候选。只要这组候选的单调性被破坏，就可以确定某些答案已经出现。
