...
哈希表解决的是“根据 key 快速找到状态”的问题。去重、计数、两数之和、缓存查询都需要把线性查找改成按 key 定位。它快的原因来自平均情况下接近 O(1) 的查找，而不是因为它跳过了数据建模。
...
print(freq)
print("go" in freq, freq["go"])
```

预期输出：

```text
{'go': 3, 'python': 2, 'rust': 1}
True 3
```

## 输出怎么读

`freq` 保存每个单词的出现次数；`"go" in freq` 是一次 key 查询；`freq["go"]` 直接读到累计状态。没有哈希表时，每次查询都要重新扫描列表。

## 常见错误

1. 忘记初始化计数，导致第一次出现时报 KeyError。
2. 用可变对象当 key，破坏哈希稳定性。
3. 只记住平均 O(1)，忽略哈希冲突、扩容和内存开销。
