...
第一个误判是只检查图文件存在。图存在，不代表 claim 指向了正确的 raw rows、分析脚本、实验边界和 caveat。

第二个误判是把强词交给人工记忆处理。`best`、`guarantee`、`always` 这类词应该进入 review finding；即使最终保留，也要有明确的 baseline、范围和证据。

第三个误判是让图表审查工具接管统计判断。这个工具只检查结构化链接和发布前阻断项；统计检验、误差线、baseline 公平性和图形表达仍需要单独审查。

## 可以怎样练习

拿一张已有实验图，给它补四个 ID：`figure_id`、`table_id`、`evidence_id`、`claim_id`。然后故意删除 raw rows 路径或把 claim 改成更强的表述，观察 gate 应该生成 block 还是 review。这个练习能训练读者把图表当成证据链的一环，而不是孤立插图。

## 参考

- The Turing Way reproducible research guide: <https://book.the-turing-way.org/reproducible-research/reproducible-research/>
- FAIR principles overview: <https://www.force11.org/group/fairgroup/fairprinciples>
