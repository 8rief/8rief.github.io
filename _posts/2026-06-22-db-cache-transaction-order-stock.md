...
下单包含一组必须保持一致的状态变化。创建订单头后，订单明细要能追溯到它；扣减库存后，库存不能因为后续明细写入失败而凭空减少；任何一个 SKU 不存在或库存不足，整次下单都应该失败并回到执行前状态。

事务解决的核心问题是原子性。应用把同一次业务动作包含的 SQL 放进同一个边界，数据库负责在成功时提交，在异常时回滚。这样测试可以围绕业务事实写断言：订单数、明细数、库存总量和收入要同时成立。

事务边界也帮助你决定函数接口。`place_order` 的输入是顾客和若干 `(sku, quantity)`，输出是一个订单 id；函数内部可以查询单价、扣库存、写明细，但调用者不需要知道每条 SQL 的细节。调用者只需要知道：函数成功返回时，订单完整存在；函数抛错时，数据库没有留下半成品订单。
...
`order_count=2` 表示两次 `place_order` 都提交成功。`sqlite_order_item_rows=4` 对应四个购买项：Backpack、Pen set、Notebook、Bottle。`total_revenue_cents=17790` 可以手算验证：`4599*2 + 499*3 + 1299*4 + 1899*1 = 17790`。`total_stock=158` 来自补货后库存 168 减去卖出的 10 件。

这四个数字要一起看。如果订单数正确但明细行少，说明明细写入路径有问题；如果收入正确但库存错误，说明扣库存或补货路径有问题；如果异常路径没有回滚，通常会出现订单头多一行而明细或库存不匹配的情况。
