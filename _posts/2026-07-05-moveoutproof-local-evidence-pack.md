...
退租争议里最麻烦的常常是材料散：入住照片在相册里，退租照片在另一个文件夹，房东消息在聊天记录里，押金扣款又出现在邮件或账单里。真正需要沟通时，用户很难快速回答三个问题：什么时间发生了什么、有哪些照片或文件能对应上、对方还缺哪些支持材料。
...
这里最重要的是“证据 ID 能对上问题”。比如一项清洁费扣款，如果已有厨房和浴室退租照片，报告会把这些照片列在 matching evidence 里，同时把发票、清洁前照片等缺失材料列为 missing proof。
...
MoveOutProof 面向的是退租前后的证据整理：照片、清单、沟通记录和时间点经常散落在不同位置，争议发生时很难还原顺序。工具的设计目标是生成本地证据包和时间线，不把材料上传到第三方服务。
...
`manifest.json` 记录证据条目、文件 hash、采集时间和备注；`timeline.csv` 按时间排序；`report.md` 给出人类可读摘要。hash 的作用是证明文件在打包后没有被静默改动。

## 可复现示例

```bash
python3 moveoutproof.py init sample-pack
python3 moveoutproof.py add-photo sample-pack door.jpg --note "front door before handover"
python3 moveoutproof.py report sample-pack --output sample-pack/report.md
```
...
items=1
timeline_entries=1
report_ready=sample-pack/report.md
status=ok
```

## 输出怎么读

`items=1` 表示证据包当前收录一条材料；`timeline_entries=1` 表示它已进入时间线；`report_ready` 是后续人工检查和导出的入口。

## 常见误判

第一个误判是只保留照片，不记录时间和对应房间。没有时间、位置和说明的照片很难和具体扣款项对应，报告也无法解释它支持哪一句沟通请求。

第二个误判是把照片数量当成证据质量。退租证据更需要覆盖关键槽位：搬入前、搬出后、对方维修或清洁后的材料，以及每个扣款项对应的缺失证明。

第三个误判是把中性消息写成结论。MoveOutProof 应先列出“已有证据”和“仍需对方提供的依据”，避免替用户断言扣款是否合法。

## 可以怎样练习

用合成文件名写一个四条记录的小 pack：`kitchen-before.jpg`、`kitchen-after.jpg`、`deposit-statement.pdf`、`message-request.txt`。练习目标是让一项清洁费扣款能连接到已有照片，同时明确还缺什么材料，例如维修发票、清洁前照片或扣款计算依据。

## 边界

这个工具只帮助整理本地证据，不提供法律结论，也不判断照片内容真实性。正式争议仍需要根据合同、当地规定和实际沟通记录处理。
