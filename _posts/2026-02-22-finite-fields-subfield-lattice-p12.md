...
子域格把“一个元素到底可能生活在哪一层域里”变成整除关系。引入 gcd/lcm 规则后，交和共同生成域可以直接计算。
...
assert gcd(4, 6) == 2 and lcm(4, 6) == 12
assert gcd(2, 3) == 1 and lcm(2, 3) == 6
assert gcd(6, 9) == 3 and lcm(6, 9) == 18
```

本次实验输出摘录：

```text
subfield_gcd_lcm_examples={'gcd_4_6': 2, 'lcm_4_6': 12, 'gcd_2_3': 1, 'lcm_2_3': 6, 'gcd_6_9': 3, 'lcm_6_9': 18}
```

## 输出怎么读

`gcd_4_6=2` 表示 `GF(p^4)` 与 `GF(p^6)` 的交是 `GF(p^2)`；`lcm_4_6=12` 表示它们共同生成 `GF(p^12)`。

## 状态变化

状态变化是：先把子域节点映射成指数；包含关系转成整除；两个节点求交时取 gcd，向上合成时取 lcm；最后再把指数翻译回有限域记号。

## 练习或延伸

在 `GF(p^18)` 中计算 `GF(p^6)` 和 `GF(p^9)` 的交与共同生成域，并画出相关局部格图。
