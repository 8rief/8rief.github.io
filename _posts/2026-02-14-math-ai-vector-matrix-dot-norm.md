...
向量代码最常见的隐蔽错误，是 shape 合法但语义轴写反。例如矩阵每一行究竟表示输出方向还是输入样本，必须在乘法前说明。本文从一组可手算数字建立运算、shape 和几何意义的对应关系。
...
点积还满足 `a·b=||a|| ||b|| cos(theta)`。因此 cosine similarity 是 `a·b/(||a||||b||)`：它消去长度，只保留方向夹角。若任一向量为零，分母为零，代码必须显式定义或拒绝这种输入。
...
lab 与文章都使用 `A=[[2,1],[1,3]]`。如果第二行误写成 `[-1,3]`，第二个输出会变成 9；这类“代码能运行、数字不一致”的错误应由断言阻止。

```python
assert a.shape == b.shape == (2,)
assert A.shape == (2, 2)
assert np.isclose(norm_a, 5.0)
assert np.allclose(projection, [0.8, -0.4])
assert np.allclose(mapped, [10.0, 15.0])
```

## 投影留下了什么

令残差 `r=a-proj_b(a)=(2.2,4.4)`。投影的关键性质是残差与方向 `b` 正交：
...
预期结果接近 `0.0`，因为 `2.2×2+4.4×(-1)=0`。最小二乘、正交分解和 PCA 都会复用“投影 + 正交残差”这个结构。

矩阵乘法则把多个点积一次完成。`A.shape=(m,n)`、`a.shape=(n,)` 时，结果 shape 为 `(m,)`；共享的 `n` 维被收缩，每一行产生一个输出。
...
预期分别为 `sqrt(5)`、`2` 和 `(1,0)`。再把 `b` 换成零向量，给函数增加明确的输入错误，避免除零产生 `nan`。
