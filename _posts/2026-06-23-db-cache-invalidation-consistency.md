...
TTL 只能回答“旧数据最多保留多久”，不能回答“刚写入的数据什么时候可见”。对库存报表来说，下单成功后收入、销量、库存都会变化。如果仍然返回下单前的缓存，页面显示就会落后于数据库事实。

显式失效解决的核心问题是写后可见性。应用知道某个写操作影响了哪些缓存结果，就在写成功后删除对应 key。下一次读取会按 cache-aside 流程回到数据库，生成新的报表。对小项目来说，少量清晰 key 加明确失效点，比引入复杂缓存系统更容易保证正确。

失效点应该放在事务成功之后。若事务中途失败却提前删缓存，通常只是多一次 miss；若事务成功后忘记删缓存，用户会读到旧结果。更稳妥的服务层做法是：事务函数负责数据库原子写入，外层服务在确认成功返回后统一失效相关 key。
...
`cache_invalidations=1` 表示代码确实删除过一次 `category_report`。这次删除发生在 Alice 的订单提交之后、重新读取报表之前。随后生成的分类报表包含最新订单数据：
...
这些数字可以反查依赖关系。`bags` 的收入来自两个 Backpack，库存从 18 变成 16；`stationery` 的收入来自 3 套 Pen set 和 4 本 Notebook；`gear` 的收入来自 1 个 Bottle，Mug 只是库存增加，没有被卖出。若删除失效调用，第二次读取可能继续返回下单前的旧报表，缓存计数会看起来“更好”，但业务结果变差。

因此，缓存命中率必须和业务新鲜度一起看，单独追求命中率会掩盖陈旧数据。
