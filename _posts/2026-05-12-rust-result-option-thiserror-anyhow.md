...
## 为什么需要 Result、Option 和分层错误

可靠 CLI 的关键不只是成功路径。日志行可能缺字段，level 可能拼错，`latency_ms` 可能不是数字，配置阈值也可能越界。`Option` 和 `Result` 把这些可能性写进类型，让调用者必须面对缺失和失败。

`thiserror` 适合核心库，因为 parser 的错误分支需要被测试精确断言；`anyhow` 适合入口层，因为读文件、解析 TOML、写报告时更关心“哪一步、哪个路径失败”。两层混在一起会让测试和用户错误信息都变差。

本项目的策略是：解析器返回具体 `ParseLogError`，文件边界用 `with_context` 补路径。这样单元测试能断言 `InvalidLevel("NOTICE")`，CLI 用户也能看到是读配置、解析日志还是写报告失败。
...
```text
test parser::tests::rejects_unknown_level ... ok
test config::tests::validates_error_rate_range ... ok
```

第一条说明非法 level 会进入明确的解析错误分支；第二条说明配置里的 `error_rate_warning` 必须在 0 到 1 之间。它们比只跑成功样例更有价值，因为可靠工具必须能解释失败。

报告里的 `slow_events=1` 来自 `latency_ms: Option<u64>`：只有存在延迟字段且大于等于配置阈值的事件才算慢事件。缺失延迟不是解析错误，非法数字才是解析错误；这就是 `Option` 和 `Result` 的边界差异。
