...
最大流不是一次性选若干条路径。早期增广可能占用了后来更好的通道，算法需要能撤销一部分旧选择。残量网络用反向边表示这种撤销能力：沿正向边发送 f，正向剩余容量减少 f，反向剩余容量增加 f。

Dinic 相比逐条找增广路的朴素方法，进一步把当前最短层次上的增广路径批量处理。一次 BFS 建层之后，DFS 不断找路直到该层次图被“堵住”，减少反复搜索相似路径的成本。
...
第一阶段可以发送 `0->1->3->5` 上的 12，以及 `0->2->4->5` 上的 4，共 16。之后这个层次图里到汇点的路被堵住。下一次 BFS 会利用剩余容量找到 `0->2->4->3->5`，再发送 7，总流量到达 23。
...
这个可达侧还能核对最小割容量。令 `S={0,1,2,4}`，`T={3,5}`，原图中从 S 指向 T 的边是：

```text
1->3(12), 4->3(7), 4->5(4)
```

容量和为 23，正好等于最大流。最大流和最小割在同一个数字上相遇，是发布结果时最有说服力的交叉检查。
...
一般图上 Dinic 的常见上界是 `O(V^2 E)`。在二分图匹配等特殊网络上有更强界。随机测试使用 seed `20260625` 生成 200 个小有向图，规模 `2..12`，容量 `1..20`，并与 Edmonds-Karp 完全对照。实际使用时，Dinic 通常比逐条最短增广路的 Edmonds-Karp 快很多。

## 输入和实现边界

- 当前实现是有向容量网络；无向容量边要拆成两个方向，语义需由题目决定。
- 容量必须非负。`add_edge` 中的反向边初始容量为 0，用于残量回退。
- DFS 使用递归，极深网络可改成迭代或确保栈空间足够。
- `max_flow` 会消耗图中的残量状态；如果要重复计算，需要重新建图。
- `mincut_reachable` 必须在最大流计算之后调用，含义才是某个最小割的源侧。
...
**在原图上找最小割可达集。** 最大流结束后要沿残量容量大于 0 的边走，而不是沿原始边走。
