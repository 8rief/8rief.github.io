...
程序需要用同一套读写接口处理普通文件、终端、管道和 socket。内核给每个已打开对象分配一个进程内整数索引，程序便可以对这个索引调用 `read` 或 `write`。对象的真实类型和连接关系由打开与重定向阶段决定。

这层间接使 shell 可以在不修改程序的情况下组合工具。一个只会向描述符 `1` 写文本的程序，既能把文本显示在终端，也能由 shell 把它接到文件或下游程序。日志采集、构建管道和测试证据都依赖这个边界。
...
在 Linux 上还可以直接查当前 shell 的描述符连接：

```bash
for fd in 0 1 2; do
  printf 'fd %s -> %s\n' "$fd" "$(readlink "/proc/$$/fd/$fd")"
done
```

交互终端中可能看到 `0`、`1`、`2` 都指向同一个 `/dev/pts/<n>`。在 CI、IDE 或管道内运行时，它们可能指向 `pipe:[<id>]` 或其他对象。路径和数字会变，关键是确认每个描述符当前接到什么。
...
shell 通常从左到右处理重定向。下面两条命令的连接结果不同：

```bash
python3 -c 'import sys; print("out"); print("err", file=sys.stderr)' > all.txt 2>&1
python3 -c 'import sys; print("out"); print("err", file=sys.stderr)' 2>&1 > out.txt
```

第一条先让 `1` 指向 `all.txt`，再让 `2` 复制当时的 `1`，两者都进入文件。第二条先让 `2` 复制原来连向终端的 `1`，随后只把 `1` 改到 `out.txt`；`err` 仍然走原来的连接。这是运算顺序带来的必然结果。
...
管道还有两个重要的运行语义。管道缓冲区有限，下游读取过慢时，上游写入可能阻塞，形成背压。当所有写端都关闭且缓冲区已读完，读端才看到 EOF。如果某个进程无意中保留了写端，下游就可能一直等不到 EOF。
