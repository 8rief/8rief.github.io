...
服务端进程调用 `socket()` 创建一个内核 socket 对象，再调用 `bind()` 把它绑定到本地地址和端口，最后调用 `listen()` 让它进入监听状态。监听状态由内核 TCP 栈维护，所以 `ss` 可以看到它；应用只持有对应 socket。
...
**只看端口号，不看绑定地址。** `127.0.0.1:18180` 和 `0.0.0.0:18180` 的暴露范围不同；绑定地址的范围可以继续看[服务绑定和暴露边界](/computer-science-teaching/2026/05/01/linux-service-binding-exposure.html)。
...
## 为什么要引入 socket、端口和 loopback

本机网络调试需要一个稳定模型：socket 是进程使用的通信端点，端口把连接交给具体服务，loopback 让请求不离开本机。理解这三者后，才能解释为什么浏览器访问 `127.0.0.1:8000` 会到达本地开发服务。
