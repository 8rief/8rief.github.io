...
很多 Git 问题的表面现象是“我的修改不见了”或“提交里没有文件”。实际原因往往是修改只在 working tree、已经进了 index、或已经成为 commit。三层状态不分清，命令就像随机改变文件。

可以把 Git 的本地工作看成一条证据链：先在目录里编辑文件，再选择哪些内容进入下一次提交，最后把这次选择保存成不可变快照。`status`、`diff`、`add`、`commit` 都是在这条链上移动或观察状态。
...
git ls-tree --name-only HEAD -> README.md, src
```

这说明 `HEAD` 指向提交对象，提交对象再指向一棵目录树。文件内容和目录结构已经进入仓库对象库，而不只存在于当前目录。
...
```text
8a7426002b0f203fb48f24e8e4679baed238709f
```

完整哈希用来唯一定位对象；日常阅读日志时通常显示前 7 到 12 位短哈希即可。
...
- `?? README.md`：文件只在 working tree 中，Git 还没有跟踪。
- `A  README.md`：新增文件已经放入 index，下一次提交会保存它。
- 提交后 `git status --short` 无输出：working tree、index、`HEAD` 三者一致。
...
这四条命令分别回答：哪些路径变化了；工作区还有什么没暂存；暂存区准备提交什么；最近提交是什么。
...
5. **只记命令顺序。** 更可靠的做法是每运行一步都用 `status` 或 `diff` 验证状态变化。
