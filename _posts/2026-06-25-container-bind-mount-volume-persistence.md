...
容器应该可以被删除、重建和替换。这个特性让部署更可控，但也带来一个直接问题：如果业务状态只写在容器自己的可写层里，容器删除后状态就会消失。持久化边界解决的核心问题是把可替换的运行实例和需要保留的数据分开。

bind mount 把宿主机上的明确目录挂到容器内路径。对初学者来说，它的优势是可观察：服务写入 `/data/state.json`，宿主机就能在 `data/runtime/state.json` 看到同一个文件。volume 更适合由 Docker 管理存储位置，但学习第一步先用 bind mount 更容易理解数据从哪里来、到哪里去。

本 lab 让第一次容器写入 2 次访问，然后停止容器、重新启动，再写入 2 次。若计数从 2 变成 4，说明状态没有跟随旧容器一起消失。
...
第一次 `visits_after_smoke=2` 表示新状态文件记录了两次访问。容器停止后，`--rm` 会删除旧容器实例；第二次启动仍然得到 `visits_after_smoke=4`，说明新容器读取到了宿主机挂载目录里的旧 `state.json`，并继续累加。

`state_ready=data/runtime/state.json` 是人工检查入口。打开这个文件应能看到 `visits` 和最近事件列表。如果文件不存在，优先检查 `-v` 参数、容器内 `DATA_DIR` 是否等于 `/data`，以及应用用户是否有写入权限。
