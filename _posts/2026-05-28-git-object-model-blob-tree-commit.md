...
如果你想跟着敲一遍，可以在临时目录里创建一个只含两个文件的小仓库：

```bash
cd /tmp
rm -rf git-object-demo
mkdir git-object-demo
cd git-object-demo
git init
mkdir -p src
printf '# demo\n' > README.md
printf 'print("hello git object")\n' > src/app.py
git add README.md src/app.py
git commit -m 'initial snapshot'
```

这组命令做了五件事：进入临时目录；创建一个空仓库；写入一个顶层文件和一个子目录文件；把 working tree 内容放入 index；最后生成第一个 commit。后面的 `cat-file` 和 `ls-tree` 都只读取这个仓库自己的对象库，不会修改你的其他项目。
