...
一个 Python 脚本最开始通常只服务作者本人：改变量、运行文件、看打印结果。项目一旦需要被重复执行、被别人调用、被测试记录行为，脚本入口就需要稳定下来。CLI 边界解决的正是这个问题：参数从哪里来，输出写到哪里，失败时怎么报错，哪些结果适合给人看，哪些结果适合给脚本读。

Typer 的职责是把命令行参数转成清楚的函数调用，让扫描、哈希、JSON/CSV 写入仍然留在普通模块里。这样 CLI 可以薄，核心逻辑可以被单元测试直接调用。

这也是从脚本走向项目的关键一步：命令入口是用户界面，模块函数是可测试边界，报告文件是可复查输出。三者分开后，后续添加 API、定时任务或批处理时才不会互相缠住。
