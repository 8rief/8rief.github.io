...
Capsule Memory Kit 是一个公开的最小实现，用来验证一种更工程化的记忆系统设计：把记忆做成一个本地文件夹胶囊，避免依赖运行时隐藏状态。任何能够读文件、运行 Python 的 agent，都可以从同一个胶囊恢复上下文、检索细节，并通过脚本化命令完成源记忆写回、索引重建和安全验证。
...
但即使没有这些扩展，当前最小版本已经能支撑一个实用原则：记忆应该成为可迁移、可验证、可重建的本地资产，不再停留在一次聊天的隐性上下文里。
...
python3 capsule_memory_kit.py remember /tmp/capsule-demo --kind operation --title "Workflow" --text "Run maintain before session end."
python3 capsule_memory_kit.py maintain /tmp/capsule-demo
