...
字节流模型要解决的核心问题是“应用消息和 TCP 读写次数没有一一对应关系”。TCP 保证字节按顺序到达，但不会告诉接收端“这是第几条业务消息”。如果应用发送两条 JSON，接收端可能一次 `recv` 读到半条、一条半，或者多条合在一起。
...
没有边界定义，连接建立成功也无法可靠解析业务数据。

## listen、connect、accept 的位置
...
`listen` 表示服务端准备接受连接；`connect` 是客户端发起连接；`accept` 返回一个新的连接 socket，用来和该客户端通信。监听 socket 继续等待下一位客户端。

## 部分读写示例

读取固定长度 payload 时，应该循环直到收够：
...
这个函数体现了 TCP 的字节流特性：一次 `recv` 返回多少字节由内核缓冲、对端发送和时机共同决定，应用不能假设固定大小。
...
第一行验证了 echo handler 的业务行为；第二行说明吞吐测量发送了 1 MiB 数据；第三行是本机 loopback 上一次观察。这个数值适合检查同一台机器、同一段代码改动后的趋势，不适合和公网、跨机器或不同系统直接比较。
...
```text
服务端是否 listen：ss -ltn
客户端是否 connect：错误是 refused、timeout 还是 reset
应用是否循环读写：是否处理部分读/写
协议是否定义边界：接收端如何知道一条消息结束
资源是否关闭：连接泄漏会让服务长期不稳定
```

这些问题分属不同层次，排查时不要只盯着业务 handler。
