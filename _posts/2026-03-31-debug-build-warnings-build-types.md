...
告警如果长期被忽略，就会变成背景噪音。真正危险的类型转换、未初始化使用或遗漏返回值混在几十条旧告警里，开发者很难注意到。把告警写进 target，并用 `-Werror` 让新增告警直接失败，可以把问题挡在运行前。

构建类型同样是调试边界。Debug 方便定位，Release 接近发布性能，sanitizer 构建负责运行时插桩。把它们混在一个 build 目录里，会让排错结论变得不可靠。
...
所以本文只声称编译、构建和符号证据已验证，不把 gdb 单步调试写成已完成内容。
...
```text
Building CXX object ...debug_lab_cli.cpp.o
Building CXX object ...test_debug_lab.cpp.o
Building CXX object ...debug_lab.cpp.o
Linking CXX static library libdebuglab.a
Linking CXX executable debug_lab_tests
Linking CXX executable debug_lab_cli
```

这说明库、CLI 和测试都使用同一套 target 配置参与了构建。

## 如何解释输出

- 看到 `-Werror` 构建通过，只能说明当前样例没有触发这些编译告警；它不能替代测试。
- 看到 Debug build 通过，说明调试构建可用；性能结论仍要用 Release 或专门 benchmark。
- 看到 `compile_commands.json` 生成，说明编辑器、clangd、静态分析工具可以读取真实编译参数。
...
5. **多个配置共用一个目录。** Debug、Release 和 sanitizer 构建应分开，避免缓存和编译选项互相污染。
