...
明细行是事实，报表行是问题答案。把所有明细读到应用里再循环，不仅传输更多数据，还容易让不同页面各写一套统计口径。把聚合写成 SQL，可以让报表、JSON 导出和测试共享同一个定义。

SQL 聚合的关键是先定分组粒度：按项目、按负责人、按状态，得到的是三种不同问题。粒度定错，数字看起来正确，含义却已经变了。
...
```text
project        | total_tickets | open_tickets | estimated_hours
---------------+---------------+--------------+----------------
blog-platform  | 4             | 3            | 5.5
lab-runner     | 3             | 2            | 6.5
research-notes | 2             | 2            | 5.0
```

`total_tickets` 是每组工单数，`open_tickets` 是每组里 `status <> 'done'` 的数量，`estimated_hours` 是该项目所有工单估算时间之和。
...
如果 `GROUP BY m.id, m.name` 改成只按 `m.name`，在当前样例里结果可能一样，但一旦允许重名成员就会合并错误。实际开发中优先按主键分组，再把名称作为展示列。

## 聚合函数的几个边界

- `COUNT(t.id)` 只统计非 NULL 的工单 id；在 `LEFT JOIN` 下，没有工单的项目不会被误计为 1。
- `SUM(CASE WHEN ... THEN 1 ELSE 0 END)` 把条件计数写得很明确，适合初学者阅读。
- `ROUND(SUM(...), 1)` 是展示层取舍；如果金额或精确计量场景，不要随意用浮点和四舍五入。
- `WHERE` 发生在分组前，`HAVING` 用于过滤分组后的结果。
...
5. **忘记 LEFT JOIN 对聚合的影响。** 需要保留空项目时，`COUNT(*)` 和 `COUNT(t.id)` 的含义不同。
