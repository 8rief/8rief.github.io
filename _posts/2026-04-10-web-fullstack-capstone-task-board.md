...
前面的文章分别解释页面、fetch、服务器、持久化、输入校验和测试。结课项目把它们合成一个能实际展示的小系统。一个任务面板虽然简单，但已经包含 Web 项目的核心闭环：用户输入、请求、响应、状态保存、界面更新和回归验证。
...
3. JSON 文件能保存状态。
4. 错误路径能返回 400 或 404。
5. 测试和 smoke 能复跑同一条链路。

这个目标比“写一个页面”更完整，也比“一开始上框架和数据库”更适合零基础学习。
...
- 展示入口：浏览器打开 `http://127.0.0.1:3000`，手动新增、完成、删除任务。
- 机器证据：`reports/smoke-report.json` 记录 `created_id=3`、`invalid_title_status=400`、`smoke_status=ok`。
- 请求证据：`reports/api-transcript.ndjson` 逐行记录每个 HTTP 请求和状态码。

如果要向别人展示，先打开页面，再展示 smoke 报告。页面说明项目可见，smoke 报告说明它有真实请求链路支撑。

## 下一步扩展边界

继续扩展时，建议一次只替换一个部件：

1. 用 Express 替换 `node:http`，保持 API 路径和响应格式不变。
2. 用 SQLite 替换 JSON 文件，保持前端调用方式不变。
3. 用 Vite 或 React 替换静态前端，保持后端 API 不变。
4. 用 Docker 或 Pages/Workers 做部署时，新增部署和健康检查证据。

每一步都要保留测试和 transcript。这样扩展项目时，能判断是新部件出问题，还是原有 API 契约被破坏。
