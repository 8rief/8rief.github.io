...
- [C++ 程序如何从源码变成可执行文件](/computer-science-teaching/2026/04/07/cpp-build-pipeline-source-to-executable.html)
- [头文件、源文件和 undefined reference：C++ 多文件构建的真实边界](/computer-science-teaching/2026/04/26/cpp-headers-source-linking-undefined-reference.html)
...
这篇文章建立了 RAII 的最小工程模型：资源必须有 owner，owner 的析构函数负责释放，作用域决定释放时机。进入 [`unique_ptr`、`shared_ptr` 和 raw pointer](/computer-science-teaching/2026/04/27/cpp-ownership-smart-pointers.html) 时，这个模型会推广到动态对象所有权：C++ 代码里“能拿到地址”不等于“拥有对象”。
