...
`0.0.0.0` 是服务端 bind 时的通配地址，客户端访问时仍然要使用具体地址。客户端仍然访问具体地址，例如 `127.0.0.1` 或主机 IPv4。
...
绑定地址决定服务从哪些网络入口接收连接。`127.0.0.1` 只接受本机访问，`0.0.0.0` 会监听所有可用 IPv4 接口。这个选择直接影响调试便利性和暴露面。
