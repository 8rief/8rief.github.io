...
一条 `docker run` 命令适合教学第一步，但参数会迅速变长：image 名、容器名、端口、环境变量、挂载、健康检查、重启策略和依赖服务都可能出现。把这些参数只留在命令历史里，复现和审查都会变困难。Compose 解决的核心问题是把部署参数写成可版本化文件。

Compose 文件也是团队沟通边界。读者不用猜这项服务暴露哪个端口、写哪个目录、需要哪些环境变量；这些都在 `compose.yaml` 中集中出现。`docker compose config` 再把 YAML 解析为规范化配置，帮助你在真正启动前发现缩进、字段名和变量问题。

本 lab 仍然保留 `docker run`，因为它能让初学者看清每个参数的含义；Compose 则作为收束步骤，把已经理解的参数沉淀成文件。这个顺序更适合学习。
...
lab 不直接把完整 `docker compose config` 打到博客正文里，而是把检查结果收进报告：
...
`true` 表示 Compose 能解析 `compose.yaml`，其中 `build.context`、`image`、`ports`、`environment`、`volumes` 和 `healthcheck` 的语法都被 Docker Compose 接受。这个检查不能证明服务一定健康，但能证明部署说明文件本身可读。

如果 `docker compose config` 失败，错误通常指向 YAML 缩进、字段位置或字符串类型。例如端口映射建议写成带引号的 `"127.0.0.1:18080:8080"`，避免 YAML 把冒号结构解释成其他形式。配置通过后，再进入 `docker compose up --build` 和 smoke 测试。
