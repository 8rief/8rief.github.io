...
没有事务时，多步写入会留下半成品。比如先插入工单，再插入事件；如果第二步失败，系统里就出现了没有创建事件的工单。这样的数据不一定立刻报错，但审计、回放和问题定位都会变困难。

事务把一组语句变成一个结果：全部成功才提交；任意一步失败就回滚。对初学者来说，先记住这个业务含义，比先背隔离级别更重要。
...
Python `sqlite3` 中，lab 用连接对象的上下文管理器表达这个边界：

```python
with conn:
    conn.execute("INSERT ...")  # 第一步成功
    conn.execute("INSERT ...")  # 第二步失败
```

异常离开 `with` 块时，事务回滚；没有异常时提交。不同语言 API 名字会变，但边界思路相同。
...
lab 中 `after` 仍为 9，说明失败路径没有污染数据库。
...
`INSERT OR IGNORE` 让初始化可以复跑。真正的生产迁移还需要按版本保存脚本、审查不可逆变更，并在发布前备份数据；本文只保留入门项目最需要的版本意识。

## 事务的使用边界

- 把同一个业务动作中的多条写入放在一个事务里。
- 不要在事务中等待用户输入或外部慢请求，避免长时间持锁。
- 失败要向调用者返回清楚错误，不要把异常吞掉后继续运行。
- SQLite 要启用 `PRAGMA foreign_keys = ON`，否则外键失败路径无法验证。
...
5. **只看成功提交。** 事务最该测试的是中间一步失败时是否真的回滚。
