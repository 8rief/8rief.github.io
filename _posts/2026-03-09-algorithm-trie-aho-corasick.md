...
如果有 P 个模式串，逐个 KMP 或逐个查找会反复扫描同一段文本。Trie 共享了模式之间的前缀，但普通 Trie 在文本失配时仍不知道该跳到哪里，只能重新尝试起点。

Aho-Corasick 把每个 Trie 节点的“最长可用后缀”预先连好。搜索文本时，状态始终代表当前文本后缀中仍可能扩展成某个模式的最长前缀，因此每个字符只驱动一次自动机转移，并一次性报告所有以当前位置结尾的模式。
...
以模式 `he, she, his, hers` 为例，`she` 节点的失败链接指向 `he`，因为 `she` 的最长真后缀且仍是模式前缀的是 `he`。因此读到 `ushers` 的第 3 个字符 `e` 时，状态到达 `she`，既要报告 `she@1`，也要通过输出继承报告 `he@2`。
...
i=1, 读到 s：状态表示前缀 "s"
i=2, 读到 h：状态表示前缀 "sh"
i=3, 读到 e：状态表示 "she"，输出 she@1，并继承 he@2
i=4, 读到 r：沿 hers 继续
i=5, 读到 s：输出 hers@2
```

所以排序后的输出为 `(she@1) (he@2) (hers@2)`。另一个测试用 `a, aa, aaa, b` 匹配 `aaaaab`，会产生大量重叠答案，用来检查输出继承和位置计算。
...
`ushers` 中同时出现 `she`、`he` 和 `hers`。其中 `he` 是 `she` 的后缀路径上的输出，`hers` 又从同一位置继续向后匹配。实验还构造了 `a`、`aa`、`aaa` 这类高度重叠模式，用朴素搜索逐位置对照；所有 `(起点, 模式编号)` 二元组排序后必须完全一致。
...
如果字母表很大，固定数组会浪费空间，构建时的 `alphabet_transition_cost` 也会变高。此时可以改成哈希表或压缩转移，但搜索常数和实现复杂度会变化。

## 输入和实现边界

- 当前代码只接受小写 `a-z`。插入或搜索其他字符会产生非法下标，生产代码必须验证输入或改用映射表。
- 必须在加入全部模式后调用 `build()`，再执行 `search()`。
- 输出位置是模式在文本中的起始下标；若要结束下标，可用 `i` 或 `start + len - 1`。
- 多个模式相同或空模式需要额外定义语义，当前教学代码不覆盖。
...
**只报告当前节点的 out。** 失败链上的模式也可能在当前位置结束，必须在构建时继承或在搜索时沿链收集。
