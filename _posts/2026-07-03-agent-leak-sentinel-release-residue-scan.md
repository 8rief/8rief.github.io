...
如果报告出现 high/critical，正确动作是删除相关文件、轮换可能泄露的凭据，再重新生成公开材料；删除报告本身不能消除泄漏。平台 secret scanning 仍然应该打开；本地预检只是把问题尽量前移。
...
先判断哪些文件应该允许公开，哪些文件应该被阻断，再把判断写成规则。重点不在于 regex 写得多复杂，而在于三件事是否清楚：规则命中什么、报告能不能暴露敏感片段、命中后发布流程应该停在哪里。
...
Agent Leak Sentinel 解决公开前的最后一类卫生问题：仓库里可能残留任务笔记、临时路径、会话片段、测试 token 形状或本地缓存。它的目标是发布前扫描，而不是替代人工代码审查。
...
```yaml
rules:
  internal_notes: ["internal-run-notes.md", "draft-notes/"]
  local_paths: ["<user-home>/", "<local-workspace>/"]
  secret_shapes: ["ghp_", "sk-"]
  caches: ["__pycache__", ".pytest_cache"]
```

扫描器输出文件、行号、规则名和建议处理方式。高风险命中应阻断发布；低风险命中可以进入人工确认列表。

## 可复现示例

```bash
python3 leak_sentinel.py scan ./public-repo --output reports/leak-scan.json
```
...
`high_risk=0` 表示没有命中明确阻断项；`review_needed=2` 表示仍有两处需要人工判断。发布门不能只看退出码，还要看命中规则是否属于真实风险。

## 边界

规则扫描会有误报，也可能漏报语义型秘密。真正公开前仍要结合 `git diff`、依赖文件和生成产物做人工抽查。
