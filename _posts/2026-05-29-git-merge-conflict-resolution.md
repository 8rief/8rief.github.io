...
冲突表示 Git 缺少足够语义来判断最终内容。两个分支改了同一段文本，机器只能告诉你冲突发生在哪里；最终状态是否应该保留一边、合并两边，或重写成第三种表达，需要人根据代码或文档语义决定。

单独学习冲突处理，是为了避免两种危险习惯：看到 `<<<<<<<` 就随手删一边，或者用强制命令把冲突状态抹掉。可靠流程应当保留证据链：冲突文件、双方提交、最终编辑、测试结果和 merge commit。

这篇实验故意制造很小的文本冲突，是因为小冲突能看清完整状态变化。真实项目里的冲突更大，但处理顺序相同。
