...
> 公开入口：本文给出架构、文件协议和命令接口；仓库暂不作为公开依赖。
...
Capsule Memory Kit 是本文展示的最小实现形态，用来验证一种更工程化的记忆系统设计：把记忆做成一个本地文件夹胶囊，避免依赖运行时隐藏状态。任何能够读文件、运行 Python 的 agent，都可以从同一个胶囊恢复上下文、检索细节，并通过脚本化命令完成源记忆写回、索引重建和安全验证。
...
其中只有 `memory/` 是人工维护的源记忆，`state/current/` 是生成状态。这个边界很重要：agent 通过 `remember` 写入源记忆，通过 `maintain` 重建 runtime pack 和 index。生成包属于派生结果，禁止直接编辑。
...
`agent-memory-core.json` 是机器入口。它的核心目标是给 agent 一个稳定协议：
...
```json
{
  ...
  "memory_targets": {
    ...
    "operation": "memory/operations.md"
  },
  "maintenance": {
    "write": "python3 capsule_memory_kit.py remember {root} --kind {user|project|operation} --title TITLE --text TEXT",
    "conflict": "python3 capsule_memory_kit.py remember {root} --kind {user|project|operation} --title TITLE --text TEXT --conflict",
    "session_end": "python3 capsule_memory_kit.py maintain {root}"
  }
}
```

这里的核心指标是字段稳定性。一个陌生 agent 不需要理解全部历史，只要知道入口、生成层位置、写回命令、冲突策略和缓存策略，就能安全开始。
...
4. 不在启动时扫描全库。
5. 通过 `remember` 写回 durable memory。
6. 通过 `maintain` 做会话结束前维护。
7. 遇到 durable preference 冲突时请用户确认。
...
python3 capsule_memory_kit.py remember /tmp/my-agent-memory --kind project --title "Current task" --text "Ship a runnable memory workflow."
python3 capsule_memory_kit.py maintain /tmp/my-agent-memory
...
`remember` 接收结构化参数，把 `--kind user`、`--kind project`、`--kind operation` 映射到固定源记忆文件，追加内容，然后自动 build 和 validate。agent 不需要自己搜索要编辑哪个文件；长内容可以先写成 UTF-8 文件，再用 `--text-file` 传入。

`maintain` 是会话结束前的一键维护命令：build、冲突检查、validate、cache audit 一次完成。
...
如果用户说以后默认用高质量模式，而旧记忆说默认用快速模式，agent 不应该自动选择折中方案。这属于 durable preference 冲突，普通事实追加规则不适用。
...
python3 capsule_memory_kit.py remember /tmp/my-agent-memory \
  --kind user \
  --title "Pending preference conflict" \
  --text "old preference says fast mode; new preference says high-quality mode." \
  --conflict
```

只要源记忆里存在这类标记，`validate` 就会失败。这个失败代表有意设置的停机点：需要用户确认偏好，然后把冲突改成新的稳定规则。
...
扫描报告只返回路径、规则名和原因，不打印匹配值。这个设计承担预检职责，能拦住 agent 工作流里最常见的发布事故。

## 定位：本地协议模板

Capsule Memory Kit 定位为低层本地协议模板，不提供云端记忆服务或向量数据库产品能力。
...
在这个基础上，可以接不同 agent：命令行智能体、本地脚本 agent、MCP 服务，甚至一个简单 shell assistant。只要它能读 `agent-memory-core.json` 和 `AGENTS.md`，就能接入。

## 最小实现的取舍

当前版本只用 Python 标准库，不依赖外部包。好处是可复制、可审计、可离线运行。代价是它不提供高级语义检索，也不内置复杂权限模型。

这个取舍是有意的。第一版目标是跑通正确边界，功能覆盖面让位于边界清晰度：
...
- 多 agent adapter，例如命令行智能体、本地脚本 agent 和 MCP 服务；
...
即使不纳入这些能力，当前最小版本已经能支撑一个实用原则：记忆应该成为可迁移、可验证、可重建的本地资产，不再停留在一次聊天的隐性上下文里。

## 接口形态

下面是工具接口应保持的最小形态。它用于说明边界，不把当前网页伪装成已发布安装包：

```bash
python3 capsule_memory_kit.py init ./capsule-demo --build
python3 capsule_memory_kit.py remember ./capsule-demo --kind operation --title "Workflow" --text "Run maintain before session end."
python3 capsule_memory_kit.py maintain ./capsule-demo
python3 capsule_memory_kit.py validate ./capsule-demo
python3 capsule_memory_kit.py route ./capsule-demo "conflicting preferences"
...
公开仓库只保存机制，不保存真实用户记忆。这个边界需要写得很明确，因为记忆系统天然容易混入隐私材料、项目状态、会话记录和本地路径。公开版应只包含模板、schema、测试、文档和空样例。
...
python3 capsule_memory_kit.py validate /tmp/my-agent-memory
python3 capsule_memory_kit.py intake "Improve this memory setup"
```
...
```text
validation=ok
private_memory_exported=false
startup_surface=stable
status=ready_for_private_capsule
```

`private_memory_exported=false` 是关键字段：它说明公开模板没有携带真实偏好、真实项目记录、认证材料或浏览器状态。真正使用时，用户应在自己的私有 capsule 中保存 durable memory。

## 设计取舍

这个项目优先选择“可审计的本地文件夹”而不是隐藏运行时状态。好处是任何能读文件和运行 Python 的 agent 都能接入；代价是维护脚本必须严格验证哪些文件能公开、哪些文件只能留在私有 capsule。

## 常见误判

第一个误判是把所有历史都塞进启动上下文。这样看似恢复完整，实际会增加首轮成本、破坏缓存稳定性，也让隐私审计更困难。正确做法是短启动入口加按需检索。

第二个误判是把生成目录当成源记忆修改。`state/current/` 这类生成层可以重建，直接编辑会让源事实、索引和运行包分叉；稳定偏好和项目状态应该写回源记忆。

第三个误判是把“可迁移”理解成“可以公开”。可迁移只说明结构能被不同 agent 读取；真实偏好、项目状态、会话记录、浏览器状态和凭据仍然必须留在私有 capsule 里。

## 可以怎样练习

用空目录初始化一个 demo capsule，然后只写三条合成记忆：一个用户偏好、一个项目状态、一个操作规则。练习目标是确认 `route` 能只返回相关内容，`maintain` 不制造无意义 diff，`validate` 能在冲突标记存在时失败。最后检查公开模板里不应出现真实路径、真实项目名、raw session 或认证文件。

## 边界

Capsule Memory Kit 不负责替你判断某条记忆是否应该长期保存。它提供的是写入、索引、验证和冲突提示机制。真正的记忆内容仍需要按隐私、安全和项目边界人工审查。
