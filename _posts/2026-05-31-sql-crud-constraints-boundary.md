...
读错通常影响一次展示，写错会改变之后所有查询的基础事实。比如状态写成 `blocked`，未完成列表、项目汇总和工作量报表都会被迫面对一个未知状态。把约束放在数据库层，能让错误在进入表之前失败。

因此学习 CRUD 时要为每类修改明确三件事：允许改哪些列；用什么条件定位目标行；失败时调用者如何知道没有写入成功。
...
应用发出写入请求，数据库先检查表结构和约束。通过检查的行进入表；违反约束的写入失败，并把错误返回给调用者。开发代码应该把这种失败当作边界反馈，不能吞掉错误后继续运行。
...
这里用 `?` 作为参数标记，是为了使用参数化查询。实际值由应用层绑定，不拼接进 SQL 字符串。
...
参数化插入会把整段内容作为值保存，不会因为单引号或分号破坏 SQL 结构。这是实际开发中最早应该养成的习惯。
...
写操作完成后，不要只看“命令没报错”。最小检查包括：

```sql
SELECT status FROM tickets WHERE id = ?;
SELECT COUNT(*) FROM ticket_events WHERE ticket_id = ? AND event_type = 'status';
```

前者确认主表状态改变，后者确认审计事件留下来了。
...
如果测试里插入 `priority=9`，会触发 `CHECK (priority BETWEEN 1 AND 5)`。如果插入不存在的 `project_id`，会触发外键错误。不同约束对应不同问题，调试时要先读清错误来自哪一层。

## DELETE 的安全流程

删除比新增和修改更难恢复。实际开发中可以按这个流程操作：

```sql
-- 1. 预览
SELECT id, title FROM tickets WHERE status = 'done';

-- 2. 确认数量后删除
DELETE FROM tickets WHERE status = 'done';

-- 3. 记录影响行数
```

如果条件来自用户输入，仍然使用参数绑定，不拼接字符串。
...
5. **把 SQL 拼成字符串。** 参数化查询不仅防注入，也能正确处理引号、换行和特殊字符。
