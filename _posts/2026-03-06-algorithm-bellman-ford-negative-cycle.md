...
负边破坏了 Dijkstra 的“弹出即最终”性质，但不会破坏另一个事实：如果没有负环，任意最短路都可以选成简单路径，最多经过 `n-1` 条边。Bellman-Ford 不急着确认某个点已经最终最短，而是逐轮放宽“最多使用几条边”的限制。

第 k 轮结束后，所有最多 k 条边的可达路径都已经被考虑。这个模型比优先队列慢，但它能自然处理负边，并能用第 n 轮检测“路径还可以通过重复某个环继续变短”的情况。
...
固定样例的前两轮能看到传播过程。边按源码顺序扫描，初始距离为：

```text
round 0: [0, INF, INF, INF, INF]
```

第一轮从源点把信息推到 1 和 2，再经过 2、3 继续传播：

```text
round 1: [0, 2, 7, 4, 2]
```

注意 `1->4(-4)` 在本轮较早时用的是旧的 `dist[1]=6`，所以 4 暂时是 2。第二轮再扫描到 `1->4` 时，已经有 `dist[1]=2`，于是得到最终的 -2：
...
随机 DAG 覆盖 `n=2..18` 的多种规模。边只从小编号指向大编号，因此不可能有环；测试再额外加入 `0->n-1` 保证至少有一条到终点的路径。Bellman-Ford 输出必须与按顶点编号顺序做的 DAG 最短路完全一致。
...
若某一轮没有任何距离改变，可以提前退出。这个优化不会改变最坏复杂度，但在无负边或路径边数较少的图上非常有效。

## 输入和语义边界

- 本实现检测的是从源点可达的负环；与源点不连通的负环不会影响 `dist`，也不会被报告。
- 一旦报告存在可达负环，受该负环影响的点没有有限最短距离。当前结构只返回布尔值，不区分哪些点被影响。
- `INF` 选为 `long long` 上限的一部分，并在松弛前检查 `dist[u] != INF`。权重规模更大时仍要额外防溢出。
- 输入顶点编号必须在 `[0,n)`；公共接口应验证 `source` 和边端点范围。
...
**把负边等同于负环。** 负边只是使 Dijkstra 的证明失效；只要不存在可达负环，最短路仍然有有限答案。
