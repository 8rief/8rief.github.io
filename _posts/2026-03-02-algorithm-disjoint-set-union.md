...
对照过程在 `n = 1..24` 的小规模上执行随机合并，每一步都比较所有点对的连通性和组件大小。这样做的目的是防止父指针更新、组件大小更新或重复合并返回值出现偏差；小规模对照不用于性能结论。
...
并查集解决动态连通性里的两个高频操作：合并两个集合，查询两个元素是否已经在同一集合。它适合边逐渐加入、需要反复判断连通关系的场景，例如 Kruskal 最小生成树和账号合并。
