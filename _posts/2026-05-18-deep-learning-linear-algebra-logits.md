...
很多 shape 报错都源于只记住了 API，没有先写出数据流。例如 32 条样本、每条 2 个特征进入二分类线性层，输入应是 `(32, 2)`，权重参与乘法时等价于 `(2, 1)`，输出才会是 `(32, 1)`。
...
令输入 `x=[2,-1]`、权重 `w=[0.5,1.5]`、偏置 `b=-0.25`，则
...
logit `-0.75` 是实数分数；概率约为 `0.321`；若阈值为 0.5，预测标签为 0。三种对象的语义不同，代码中也应使用不同变量名。
...
```text
tensor([-0.7500,  2.7500])
tensor([0.3208, 0.9399])
```

矩阵乘法把每一行样本与同一组权重做点积，broadcasting 再把偏置加到每一行。批处理改变计算组织方式，不改变单样本公式。
...
PyTorch 保存的权重 shape 是 `(out_features, in_features)`，所以这里打印结果是：

```python
model = LinearClassifier()
print(tuple(model.linear.weight.shape))
print(tuple(model.linear.bias.shape))
print(tuple(model(torch.zeros(32, 2)).shape))
```
...
公式中常写 `XW`，而 `nn.Linear` 内部等价于 `X @ weight.T + bias`。理解这个转置，就不会把数学记号与框架的参数存储格式混为一谈。

## logits 为什么直接进 loss

概率需要落在 0 到 1 之间，logits 可以是任意实数。很多 loss 函数把 sigmoid 和对数损失合在一起做，可以提高数值稳定性。写训练代码时，把 logits 交给对应 loss，把 sigmoid 放到评估和解释阶段。

对于二分类，阈值 `sigmoid(z) >= 0.5` 等价于 `z >= 0`。训练阶段仍保留原始 logit，因为它携带距离决策边界的连续信息；若先把输出变成 0/1 标签，梯度几乎处处为零，无法用于优化。
