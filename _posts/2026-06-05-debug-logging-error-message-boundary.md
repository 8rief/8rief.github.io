...
自由文本日志读起来像人话，排查时却很难批量搜索。今天写 `warning`，明天写 `warn`，后天写 `WARN:`，脚本就很难稳定统计。结构化字段让日志先有固定骨架，再保留人类可读的消息。

初学阶段不需要直接上复杂日志系统。先把级别、组件和消息写稳定，就已经能让 grep、测试和 transcript 发挥作用。
...
```text
level=info  正常诊断
level=warn  输入可疑或部分失败
component   负责输出这条诊断的模块
message     可读说明
```

字段少，但足以定位 parser 接受了什么、拒绝了什么。
...
表示解析器拒绝了非正数 token。它给出原因类别，但没有打印完整输入，避免日志变成数据泄露渠道。

## stdout 和 stderr 边界

- 正常机器可读结果放 stdout。
- 错误消息和诊断放 stderr 或日志文件。
- 公开 transcript 中不要包含密钥、本机绝对路径和大段原始输入。
...
5. **日志字段经常改名。** 字段名是脚本契约，改名前要同步测试和文档。
