...
本地 Git 已经能记录历史，但协作需要交换历史。remote 保存大家共同认可的引用位置，例如 `main` 当前指向哪个提交。你把提交 push 上去，别人 fetch 后才能看到；别人 push 之后，你的本地 `main` 不会自动变化，需要 fetch 或 pull。

初学时用本地 bare remote 可以把网络、权限和平台规则先拿掉，只观察 Git 自身的对象和引用交换。
...
`origin/main` 是“上次 fetch 看到的远端 main”。它不是本地 `main`，也不是实时网络查询结果。只有 fetch 之后，它才会更新。
...
```text
Local before pull: 71c3f61f56ae2b9ae7129b0e18982444f09963b5
Origin/main after fetch: e101259210062e83957cf79656b7ced9d59b87ab
Local after pull: e101259210062e83957cf79656b7ced9d59b87ab
```
...
* 71c3f61 (tag: v0.1.0) Add ignore rules
```

tag 指向发布点，后续 main 前进不会带着 tag 移动。
...
第一步确认本地没有未保存修改；第二步更新远程跟踪分支；第三步看清本地和远端关系；第四步只接受快进更新。若 `--ff-only` 失败，说明双方各自有新提交，需要先决定 merge、rebase 或其他协作方案。
...
5. **fetch 后以为工作区已经更新。** fetch 只更新远程跟踪引用；要更新当前分支还需要 merge、rebase 或 pull。
