...
## 为什么需要 prefix function

字符串匹配最浪费时间的地方出现在失配之后。朴素算法会把模式串右移一位，再从开头重新比较；如果文本和模式串有很多重复前缀，这些比较会反复发生。prefix function 的作用是把“已经匹配过的一段里，哪些前缀仍可能继续成立”预先记录下来。

因此，KMP 引入 prefix function 是为了解决失配恢复问题：文本指针不回退，模式串指针直接跳到下一个仍可能匹配的位置。这个位置必须同时满足两个条件：它是已经匹配串的后缀，也是模式串的前缀。`pi[j - 1]` 正好记录这个边界长度。

学习 KMP 时不要先背模板。先问一句：当前已经匹配了 `j` 个字符，下一个字符失败后，哪些信息还可以保留？能回答这个问题，`while (j > 0 && ...) j = pi[j - 1]` 就不是魔法跳转。
...
这份输出可以这样读：`ababaca` 的 `0 0 1 2 3 0 1` 表示前五个字符里出现了长度逐步增长的可复用边界，但到字符 `c` 时边界断开；搜索结果里的 `0 2 5 7` 则说明算法没有漏掉重叠或相邻匹配。随机对照的价值在于防止模板只在手写样例上成立。
...
- [Princeton Algorithms, Part II](https://algs4.cs.princeton.edu/home/)
