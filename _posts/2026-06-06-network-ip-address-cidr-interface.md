...
CIDR 要解决的核心问题是“IP 地址需要被归到网络范围里”。路由、访问控制、本地服务绑定和日志分析都要判断一个目标属于哪个前缀。字符串前缀无法表达这个关系；`192.168.10.1` 和 `192.168.1.1` 看起来相似，但是否在同一个网络取决于二进制前缀长度。
...
`192.168.1.0/24` 表示前 24 位固定，最后 8 位变化：

```text
192.168.1.0/24     覆盖 192.168.1.0 到 192.168.1.255
192.168.1.42       属于这个网络
192.168.2.42       不属于这个网络
```

Python `ipaddress` 的输出可以直接验证：

```python
import ipaddress
net = ipaddress.IPv4Network("192.168.1.0/24")
print(ipaddress.IPv4Address("192.168.1.42") in net)
print(ipaddress.IPv4Address("192.168.2.42") in net)
```
...
读这个输出时，不要看字符串开头是否像，而要看地址数值是否落在网络范围内。

## interface 地址和目标地址的关系

`ip addr show` 里的地址描述本机接口：

```text
inet 127.0.0.1/8 scope host lo
```

这行可以读成：loopback interface `lo` 上有一个 IPv4 地址 `127.0.0.1`，它所属的前缀是 `127.0.0.0/8`。当目标是 `127.0.0.1` 时，路由选择会把它留在本机 loopback 路径上。

## 状态变化

当应用准备连接一个 IP 时，内核会做三步：

```text
目标 IP -> 匹配路由表前缀 -> 选择出口 interface 和下一跳
```

CIDR 是第二步的基础。没有前缀，系统无法知道目标应走 loopback、本地网段还是默认网关。
...
只说明本次环境里观测到 5 个接口项和 4 条路由项，不是跨机器常量。学习重点是会把 `ip addr` 的接口地址和 `ip route` 的前缀规则联系起来。
