...
CSV 文件没有强类型。`"10"`、`"1.20"`、`"2026-01-04"` 都只是文本，只有进入代码后才会被解释成整数、金额和日期。如果直接拿字符串参与计算，错误可能很隐蔽：字符串排序会让 `"100"` 排在 `"20"` 前面，浮点金额也可能出现不符合财务直觉的舍入。

schema 的作用是把“这一列应该是什么”写清楚。本 lab 的字段可以分成四类：

1. 标识字段：`order_id` 必须存在且不能重复。
2. 时间字段：`date` 必须能通过 `date.fromisoformat`。
3. 分类字段：`region`、`channel`、`product` 必须有可解释取值。
4. 数值字段：`units`、`unit_price`、`discount_pct` 必须能转换并通过范围检查。

入口边界越早固定，后面的 SQLite、JSON 和图表越稳定。
...
`wc -l data/raw/sales.csv` 的 `21` 包含 1 行表头和 20 行数据记录。`DictReader` 使用表头作为 key，因此第一条记录可以理解成：
...
这里的引号提醒我们：这些值刚读入时都是字符串。校验函数把 `units` 转成 `int`，把 `unit_price` 和 `discount_pct` 转成 `Decimal`，把 `date` 转成可验证的 ISO 日期。只有转换成功并通过范围检查，才会进入 clean 数据。
...
任何一步失败，都应该转入 reject，而不是带着不确定类型继续汇总。
