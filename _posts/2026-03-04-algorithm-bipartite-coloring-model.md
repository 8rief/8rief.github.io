...
二分图染色解决的是“能否把点分成两组，使每条边都跨组”的判断问题。它是匹配、最小点覆盖和许多约束建模问题的入口。如果染色阶段已经发现冲突，后续谈匹配就没有意义。
...
print(is_bipartite({1:[2,3],2:[1,3],3:[1,2]}))
```
...
三角形中任意两点相邻，第三条边会要求两个已经同色的点再次分到不同组，所以返回 `False`。冲突边就是证明图非二分的局部证据。

## 常见错误

1. 只从 1 号点开始染色，漏掉非连通分量。
2. 遇到已染色邻居时不检查冲突。
3. 把“没有奇环”的结论当成代码输出，却没有返回具体冲突证据。
