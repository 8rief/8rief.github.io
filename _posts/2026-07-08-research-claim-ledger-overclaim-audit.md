...
它的主要问题是边界过大。若当前材料只证明了某个语法约束下的局部泄漏闭包，或者只展示了合成 workload 上的某个指标，就不能把它写成完整 ORAM 构造。正确做法通常有两个：要么补齐证明和实验，要么把 claim 降级成 hypothesis 或 narrower result。
...
这个工具定位为发布 hygiene 工具，不承担证明系统的职责。
...
- ACM Artifact Review and Badging: <https://book.the-turing-way.org/reproducible-research/reproducible-research/>
...
Research Claim Ledger 的目标是把研究结论、证据和限制绑定在一起。它适合在写博客、README 或论文草稿前先检查：每个结论是否有来源，证据是实验、代码、论文还是推断，限制条件有没有一起写出来。
...
```yaml
claim: "baseline A is slower under this synthetic setting"
evidence_type: "measured"
evidence_path: "reports/latency.csv"
boundary: "synthetic dataset, local machine, 5 repeated runs"
confidence: "medium"
```

ledger 不追求复杂数据库，先保证每条公开结论都有证据类型和边界。这样后续写作时不会把 `medium` 置信度写成确定性结论。

## 可复现示例

```bash
python3 claim_ledger.py audit claims.yaml --output reports/claim-audit.md
```
...
`missing_boundary=1` 表示至少一条结论缺少适用范围；`overclaim_risk=1` 表示有一句话可能把局部证据写成普遍结论。修复方式是补边界或降低措辞强度。

## 常见误判

第一个误判是把 evidence path 当成证据本身。路径只说明材料在哪里；报告是否真的支持 claim，还要看数据口径、实验设置和结论措辞是否一致。

第二个误判是把 `confidence=medium` 留在账本里，却在 README 里写成确定结论。ledger 的价值就在于阻止这种跨文件的语气漂移。

第三个误判是只审查性能 claim。工具适合同样检查可用性、安全边界、复现性和产品体验类 claim，只是不同 claim 需要不同 evidence 类型。

## 可以怎样练习

从一篇自己的报告里抽出五句话：一个性能结论、一个正确性结论、一个工程可用性结论、一个限制条件、一个下一步计划。把它们都写进 ledger。练习目标是区分 `verified`、`hypothesis` 和 `planned`，不要把计划写成已经完成的结果。

## 边界

这个工具不判断论文创新性，也不替代同行评审。它只检查 claim-evidence-boundary 三者是否在写作材料中可追踪。
